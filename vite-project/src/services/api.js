...
export const migrateHistoricPayments = async (token) => {
  const response = await fetch(`${API_URL}/wallet/admin/migrate`, {
    method: 'POST',
...
export const getAdminEarnings = async (token) => {
  const response = await fetch(`${API_URL}/wallet/admin/earnings`, {
    headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }
  });
  const data = await response.json();
  if (!response.ok) throw new Error(data.error || 'Failed to fetch admin earnings');
  return data;
};
