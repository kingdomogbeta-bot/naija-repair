...
import { MapPin, AlertTriangle, Phone, X, Navigation, Clock } from 'lucide-react';
...
export default function SafetyTracker({ booking, onClose }) {
  ...
  const reportIssue = () => {
    ...
    const reports = JSON.parse(localStorage.getItem('naija_safety_reports') || '[]');
    reports.push({
      id: Date.now(),
      bookingId: booking.id,
      taskerEmail: booking.taskerEmail,
    ...
    localStorage.setItem('naija_safety_reports', JSON.stringify(reports));

    const notifications = JSON.parse(localStorage.getItem('naija_notifications') || '[]');
    notifications.push({
      id: Date.now(),
      email: 'admin@naijarepair.com',
      message: `Safety report filed for booking #${booking.id}`,
      link: '/admin/reports',
      createdAt: Date.now(),
      read: false
    });
  ...
  const call911 = () => {
    if (confirm('This will attempt to call emergency services. Continue?')) {
      ...
      reports.push({
        ...
        bookingId: booking.id,
        taskerEmail: booking.taskerEmail,
        issue: 'EMERGENCY - 911 CALL INITIATED',
        location: location,
        timestamp: Date.now(),
        status: 'emergency'
      });
      ...
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <button
              onClick={call911}
              className="py-4 bg-red-600 hover:bg-red-700 text-white rounded-xl font-semibold flex items-center justify-center gap-2"
            >
