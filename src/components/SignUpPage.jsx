        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          ...
          {submitted ? (
            <div className="text-center py-8">
              ...
              <Link to="/dashboard" className="inline-block bg-teal-600 text-white px-8 py-3 rounded-xl font-semibold hover:bg-teal-700 transition-all">Go to Dashboard</Link>
            </div>
          ) : (
            <form
              className="space-y-4"
              onSubmit={e => {
                ...
                const storedUsers = JSON.parse(localStorage.getItem('naija_registered_users') || '{}');
                storedUsers[email] = { name: fullName, email, phone };
                localStorage.setItem('naija_registered_users', JSON.stringify(storedUsers));
                
                setSubmitted(true);
                auth.login({ name: fullName, email, phone });
