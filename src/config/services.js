...
export const getServices = () => {
  const adminServices = localStorage.getItem('admin_services');
  return adminServices ? JSON.parse(adminServices).filter(s => s.active !== false) : DEFAULT_SERVICES;
};
...
export const getServiceCategories = () => {
  ...
  return adminCategories ? JSON.parse(adminCategories) : ['All', 'Home', 'Repairs', 'Moving', 'Outdoor'];
};

export const SERVICE_CATEGORIES = getServiceCategories();
export const SERVICE_NAMES = getServices().map(s => s.name);
